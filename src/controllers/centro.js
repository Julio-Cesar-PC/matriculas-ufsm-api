...
class CentroController {
    ...
    post(req, res) {
        const {nome} = req.body;
        database.insert({codigo_centro: nome}).into('Centro')
            .then(() => res.send('Centro cadastrado com sucesso!'))
            .catch(err => {
                console.log(err);
                if (err.code === '23000') {
    ...
    put(req, res) {
        ...
        database('Centro').where('codigo_centro', original)
            .then(exist => {
                if (exist.length === 0) {
                    res.status(400).send('Centro não encontrado!');
                } else {
                    database('Centro').where('codigo_centro', original).update({codigo_centro: novo})
                        .then(() => res.send('Centro atualizado com sucesso!'))
                        .catch(err => {
                            console.log(err);
    ...
    delete(req, res) {
        const {nome} = req.body;
        database('Centro').where('codigo_centro', nome)
            .then(exist => {
                if (exist.length === 0) {
                    res.status(400).send('Centro não encontrado!');
                } else {
                    database.select().from('Sala').where('centro', nome)
                        .then(data => {
                            if (data.length > 0) {
                                res.status(400).send('Centro está sendo utilizado em uma sala!');
                            } else {
                                database.select().from('Curso').where('centro', nome)
                                    .then(data => {
                                        if (data.length > 0) {
                                            res.status(400).send('Centro está sendo utilizado em um curso!');
                                        } else {
                                            database.select().from('Professor').where('centro', nome)
                                                .then(data => {
                                                    if (data.length > 0) {
                                                        res.status(400).send('Centro está sendo utilizado em um professor!');
                                                    } else {
                                                        database('Centro').where('codigo_centro', nome).del()
                                                            .then(() => res.send('Centro deletado com sucesso!'))
                                                            .catch(err => {
                                                                console.log(err);
